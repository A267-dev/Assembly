...
{
	public class FourthGenResourceLayoutTable
	{
		...
		public IEnumerable<ResourcePage> LoadPages(IReader reader)
		{
			...
			FourthGenCacheFileReference[] files = LoadExternalFiles(values, reader);
		...
		public void SavePages(ICollection<ResourcePage> pages, ICollection<ResourcePointer> pointers, IStream stream)
		{
			...
			FourthGenCacheFileReference[] files = LoadExternalFiles(values, stream);
		...
		private FourthGenCacheFileReference[] LoadExternalFiles(StructureValueCollection values, IReader reader)
		{
