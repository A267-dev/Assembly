...
{
	/// <summary>
	///     The tag + group table in a Fourth-generation cache file.
	/// </summary>
	public class FourthGenTagTable : TagTable
	{
		...
		public FourthGenTagTable(IReader reader, SegmentPointer indexHeaderLocation, FileSegmentGroup metaArea,
		...
		public override ITag AddTag(int groupMagic, int baseSize, IStream stream)
		{
			...
			var result = new FourthGenTag(index, tagGroup, SegmentPointer.FromPointer(address, _metaArea));
			_tags.Add(result);
		...
		public void SaveChanges(IStream stream)
		{
			...
			if (Interops != null && Interops.Count > 0)
			{
				...
				IEnumerable<StructureValueCollection> entries = _interops.OrderBy(i=>i.Pointer).Select(t => ((FourthGenTagInterop)t).Serialize());
				// hax
		...
		private void SaveTags(StructureValueCollection headerValues, IStream stream)
		{
			...
			IEnumerable<StructureValueCollection> entries = _tags.Select(t => ((FourthGenTag) t).Serialize(Groups, _expander));
		...
		private List<ITagGroup> LoadTagGroups(IReader reader, StructureValueCollection headerValues)
		{
			...
			return entries.Select<StructureValueCollection, ITagGroup>(e => new FourthGenTagGroup(e)).ToList();
		}
		...
		private List<ITagInterop> LoadTagInterops(IReader reader, StructureValueCollection headerValues)
		{
			...
			StructureLayout layout = _buildInfo.Layouts.GetLayout("tag interop element");
			StructureValueCollection[] entries = TagBlockReader.ReadTagBlock(reader, count, address, layout, _metaArea);
			return entries.Select<StructureValueCollection, ITagInterop>(e => new FourthGenTagInterop(e, _metaArea)).ToList();
		}
		...
		private List<ITag> LoadTags(IReader reader, StructureValueCollection headerValues, IList<ITagGroup> groups)
		{
			...
			StructureValueCollection[] entries = TagBlockReader.ReadTagBlock(reader, count, address, layout, _metaArea);
			return
				entries.Select<StructureValueCollection, ITag>((e, i) => new FourthGenTag(e, (ushort) i, _metaArea, groups, _expander))
					.ToList();
		}
