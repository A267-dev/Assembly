...
namespace Blamite.Blam.FourthGen.Structures
{
	...
	/// </summary>
	public class FourthGenHeader
	{
		private FileSegment _eofSegment;
		public int cacheSegmentAlignment;
		private IPointerExpander _expander;

		public FourthGenHeader(StructureValueCollection values, EngineDescription info, FileSegmenter segmenter,
		...
		public FourthGenInteropSection[] Sections { get; private set; }
		...
		private void LoadInteropData(StructureValueCollection headerValues)
		{
			...
			if (headerValues.HasArray("offset masks") && headerValues.HasArray("sections"))
			{
				...
				{
					if (Sections[(int)FourthGenInteropSectionType.Resource].Size == 0)
					{
						Sections[(int)FourthGenInteropSectionType.Resource].VirtualAddress = (uint)HeaderSize;
						Sections[(int)FourthGenInteropSectionType.Resource].Size = (uint)FileSize - (uint)HeaderSize;
					}
				}
			}
		...
		private BasedPointerConverter MakePointerConverter(FourthGenInteropSectionType section)
		{
