...
{
	public class FourthGenZoneSetTable : IZoneSetTable
	{
		...
		private IEnumerable<FourthGenZoneSet> ReadZoneSetTable(StructureValueCollection tagValues, string countName, string addressName, IReader reader)
		{
			...
			return entries.Select(e => new FourthGenZoneSet(e, reader, _metaArea, _expander));
		}
		...
		private void SaveZoneSetTable(IZoneSet[] sets, StructureValueCollection tagValues, string countName, string addressName, TagBlockCache<int> cache, IStream stream)
		{
			...
			List<StructureValueCollection> entries =
				sets.Select(set => ((FourthGenZoneSet) set).Serialize(stream, _allocator, cache, _expander)).ToList();
		...
		private void FreeZoneSetsInTable(StructureValueCollection tagValues, string countName, string addressName, IReader reader)
		{
			...
			foreach (StructureValueCollection entry in entries)
				FourthGenZoneSet.Free(entry, _allocator, _expander);
		}
	}
}