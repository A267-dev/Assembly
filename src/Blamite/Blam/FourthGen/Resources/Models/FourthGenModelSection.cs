...
{
	public class FourthGenModelSection : IModelSection
	{
		public FourthGenModelSection(StructureValueCollection values, IReader reader, FileSegmentGroup metaArea,
			...
			EngineDescription buildInfo)
		{
			...
			Submeshes = (from entry in entries
				select new FourthGenModelSubmesh(entry)).ToArray();
		}
