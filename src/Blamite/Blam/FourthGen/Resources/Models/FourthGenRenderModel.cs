...
{
	public class FourthGenRenderModel : IRenderModel
	{
		public FourthGenRenderModel(StructureValueCollection values, IReader reader, FileSegmentGroup metaArea,
			...
			EngineDescription buildInfo)
		{
			...
			Regions = (from entry in entries
				select new FourthGenModelRegion(entry, reader, metaArea, buildInfo)).ToArray();
		}
			...
			EngineDescription buildInfo)
		{
			...
			Sections = (from entry in entries
				select new FourthGenModelSection(entry, reader, metaArea, buildInfo)).ToArray();
		}
